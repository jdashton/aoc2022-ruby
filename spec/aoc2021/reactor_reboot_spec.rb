...
RSpec.describe ReactorReboot do
  ...
  context "with first example" do
    subject { ReactorReboot.new(StringIO.new(<<~STEPS)) }
  ...
  context "with larger example" do
    ...
    describe "#cube_volume" do
      it "find 590,784 lit cubes" do
        expect(subject.cube_volume).to eq 590_784
      end
    end
  end

  context "with part 2 example" do
    subject { ReactorReboot.new StringIO.new(<<~STEPS) }
  ...
  context "with part 2 on actual input" do
    subject { File.open("input/day22a.txt") { |file| ReactorReboot.new file } }

    it "finds 551,693 lit cubes under the rules for part 1" do
      expect(subject.cube_volume).to eq 551_693
    end
